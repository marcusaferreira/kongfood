...
import br.com.fiap.techchallenge.kongfood.domain.order.entities.*
...
class OrderRemoveProductInteractorUnitTest {
    ...
    val orderRemoveProductInteractor = OrderRemoveProductInteractor(
        orderRepository,
        orderPresenter,
        orderSearchBoundary,
        orderProductFactory
    ...
    fun `should remove an order line`(){
        Mockito.`when`(orderSearchBoundary.getOrder(orderMock!!.id)).thenReturn(
                orderMock!!)
        orderMock!!.addOrderLine(product = orderProductFactory.createOrderProduct(orderLineMockDTO!!), 1, "no onions")

        orderRemoveProductInteractor.removeOrderLine(orderMock!!.id, orderLineMockDTO!!)
